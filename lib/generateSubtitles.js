...
// 進 Vercel 的 build。
// 釘死在 3.8.1，不要抓最新版：實測過 4.x（4.2.0）版本會噴
// "Can't create a session...Missing required scale...DequantizeLinear"，
// 是 4.x 拉進來的 onnxruntime-web 版本（jsdelivr 上看到的是一個 dev/nightly
// build）跟 Xenova/whisper-tiny 這個量化過的 ONNX 模型檔不相容，不是我們
// 自己程式碼的問題。3.8.1 實測過整條 pipeline（下載模型→建立 session→
// 真的跑一段音檔）都正常。之後如果要升版，先用這個檔案手動測過一輪
// （改這個 URL 版本號，瀏覽器開發者工具看 console 有沒有噴同一種
// onnxruntime session 錯誤）再換，不要來源一有新版就跟著升。
let transcriberPromise = null;
function getTranscriber(onProgress) {
  if (!transcriberPromise) {
    transcriberPromise = import(/* webpackIgnore: true */ "https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1/+esm").then(({ pipeline }) =>
