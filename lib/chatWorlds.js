...
// its OWN copy of --chat-world-bg directly (full quality, no wash/tint over
// it — the photo itself is the whole point), rather than relying on being
// translucent enough to reveal a single shared full-viewport image sitting
// on <body> — that approach turned out to be unreliable in practice (worked
// for some panels, silently didn't for others depending on how deeply
// nested/clipped they were), where every panel painting its own independent
// copy can't fail that way since it doesn't depend on anything above it in
// the tree. background-attachment:fixed on every one of them is what makes
// the copies line up into one continuous picture instead of each showing an
// independently-cropped slice.
