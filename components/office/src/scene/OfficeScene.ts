...
export class OfficeScene {
  ...
  private desiredExternalStates = new Map<
    string,
  ...
  /** 桌面主程式的真實任務狀態；正在交接時先記錄，回座後再套用。 */
  syncExternalAgentState(id: string, state: AgentState, task?: string) {
    this.desiredExternalStates.set(id, { state, task })
    this.applyExternalStates()
  ...
  private onTick = (ticker: { deltaTime: number }) => {
    ...
    if (this.options.enableAutoWorkflow !== false) {
  ...
  private applyExternalStates() {
    this.agents = this.agents.map((agent) => {
      const desired = this.desiredExternalStates.get(agent.id)
      if (!desired || agent.mission || agent.state === 'walking' || agent.customAnimation) {
        return agent
      }
      return {
        ...agent,
        state: desired.state,
        currentTask: desired.task,
        targetX: undefined,
        targetY: undefined,
        walkPath: undefined,
        walkPathIndex: undefined,
        bubbleText: undefined,
        viewFacing:
          desired.state === 'working' || desired.state === 'thinking'
            ? ('back' as const)
            : ('front' as const),
      }
    })
