...
export default function ChatMoreMenu({ state, setters, onOpen }) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--panel-alt)", overflowY: "auto", minHeight: 0 }}>
      ...
      <div style={{ padding: "0 10px 10px" }}>
        <NavItem icon="🔄" iconBg="linear-gradient(135deg,#0d9488,#0891b2)" label="文檔轉換" sublabel="圖片・影音格式互轉" mobileTouch
          active={state.showDocConvert} onClick={() => onOpen(setters.setShowDocConvert)} />
      </div>
      <div style={{ padding: "0 10px 10px" }}>
